  ...
  view(model, action) {
    return (
      <div className="widget">
        ...
        <Counter count={model.counter1} onUpdated={state => action({type: 'CALCULATE_COUNTER_1', ...state})} />
        <Counter count={model.counter2} onUpdated={state => action({type: 'CALCULATE_COUNTER_2', ...state})}/>
      </div>
    )
  }
}
